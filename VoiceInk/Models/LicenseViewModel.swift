...
class LicenseViewModel: ObservableObject {
    ...
    private let logger = Logger(subsystem: AppConfig.shared.loggerSubsystem, category: "LicenseViewModel")
    ...
    private func loadLicenseState() {
        guard config.enableLicenseValidation else {
            ...
            // Surface any previously stored key so the UI can display it.
            licenseState = .licensed
    ...
    var canUseApp: Bool {
        ...
        if !config.enableLicenseValidation { return true }
    ...
    var usageRestrictionMessage: String? {
        switch licenseState {
        case .unlicensed, .trialExpired:
            if let purchaseURL = config.purchaseURL, !purchaseURL.isEmpty {
                return String(
                    format: String(localized: "Your trial has ended. Upgrade to %@ Pro at %@"),
                    config.appName, purchaseURL
                )
            }
            return String(localized: "Your trial has ended.")
        case .trial, .licensed:
            return nil
        }
    }
    ...
    func openPurchaseLink() {
        if let urlString = config.purchaseURL,
           !urlString.isEmpty,
    ...
    func validateLicense() async {
        ...
        guard config.enableLicenseValidation else {
            ...
            // Brief delay so the UI's loading state is observable.
