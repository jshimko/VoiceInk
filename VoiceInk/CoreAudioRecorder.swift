...
final class CoreAudioRecorder: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: AppConfig.shared.loggerSubsystem, category: "CoreAudioRecorder")
    ...
    private let audioProcessingQueue = DispatchQueue(
        label: "\(AppConfig.shared.mainBundleIdentifier).audioProcessing", qos: .userInitiated)
