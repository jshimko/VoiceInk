...
struct LicenseManagementView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            ...
            Text("This deactivates \(config.appName) on this Mac and frees a device on your license.")
        }
    }
    ...
    private var reportPanelSurface: some View {
        ReportFeedbackBottomPanel(
            ...
            onDiscord: (config.showCommunityLinks && config.discordURL != nil)
                ? {
                    if let discordURL = config.discordURL {
                        openURL(discordURL)
                    }
                    dismissReportPanel()
                }
    ...
    private var purchasePanel: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            if config.showPurchaseOptions, config.purchaseURL != nil {
                LicenseActionButton(
                    title: "Buy License",
    ...
    private var resourcesGrid: some View {
        LazyVGrid(
            ...
            if config.showDonationLink, let donationURL = config.donationURL {
                ResourceLinkRow(
                    title: "Tip Jar",
            ...
            } else if config.licensePortalURL != nil {
                ResourceLinkRow(
                    title: "Lost Key?",
...
private struct ReportFeedbackBottomPanel: View {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            VStack(spacing: 20) {
                VStack(spacing: 18) {
                    Text("Thank you for using \(AppConfig.shared.appName)")
                        .font(.system(size: 24, weight: .semibold, design: .rounded))
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)

                    Text(
                        "Have feedback, a bug report, or something that feels off? Send a note with system information by email. Every report helps make \(AppConfig.shared.appName) more reliable and easier to use."
                    )
                    .font(.callout)
                    .foregroundStyle(.secondary)
                    .multilineTextAlignment(.center)
