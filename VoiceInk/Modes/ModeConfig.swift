...
struct ModeConfig: Codable, Identifiable, Equatable {
    ...
    init(
        id: UUID = UUID(), name: String, icon: ModeIcon = .defaultIcon, appConfigs: [ModeAppConfig]? = nil,
    ...
    init(from decoder: Decoder) throws {
        ...
        appConfigs = try container.decodeIfPresent([ModeAppConfig].self, forKey: .appConfigs)
...
struct ModeAppConfig: Codable, Identifiable, Equatable {
    let id: UUID
    ...
    static func == (lhs: ModeAppConfig, rhs: ModeAppConfig) -> Bool {
        lhs.id == rhs.id
    }
}
...
class ModeManager: ObservableObject {
    ...
    func addAppConfig(_ appConfig: ModeAppConfig, to config: ModeConfig) {
    ...
    func removeAppConfig(_ appConfig: ModeAppConfig, from config: ModeConfig) {
        if var updatedConfig = configurations.first(where: { $0.id == config.id }) {
