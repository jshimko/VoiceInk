...
final class SoundPlaybackEngine: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "\(AppConfig.shared.mainBundleIdentifier).soundPlayback", qos: .userInitiated)
    private let logger = Logger(subsystem: AppConfig.shared.loggerSubsystem, category: "SoundPlaybackEngine")
