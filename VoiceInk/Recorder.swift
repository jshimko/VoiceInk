...
class Recorder: NSObject, ObservableObject {
    ...
    private let audioMeterQueue = DispatchQueue(label: "\(AppConfig.shared.mainBundleIdentifier).audiometer", qos: .userInteractive)
    /// Dedicated serial queue for hardware setup.
    private let audioSetupQueue = DispatchQueue(label: "\(AppConfig.shared.mainBundleIdentifier).audioSetup", qos: .userInitiated)
